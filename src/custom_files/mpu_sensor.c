...
int app_mpu_config(){
    ...
    //reset the things or something
    int retval = app_mpu_write_single_register(MPU_REG_SIGNAL_PATH_RESET, 0b00000111);//last three bits 1 to reset regs corresponding to last 3 bits

    if(retval){
        LOG_INF("error write singe reg?\n");
        return retval;
    }

    retval = app_mpu_write_single_register(MPU_REG_PWR_MGMT_1, 0b00000001);
    if(retval){
        LOG_INF("error write singe reg?\n");
        return retval;
    }
    ...
    retval = app_mpu_write_registers(MPU_REG_SMPLRT_DIV, data, 4);
    if(retval){
        return retval;
    }
...
int twi_init(void)
{
    ...
    if (err != NRFX_SUCCESS){
        LOG_INF("nrfx_twim_init() != success\n");
        return err;
    }
...
int read_accel_values(accel_values_t * p_accel_values_destination){
    ...
    p_accel_values_destination->x = ((raw_values[0]<<8) + raw_values[1]);
    p_accel_values_destination->y = ((raw_values[2]<<8) + raw_values[3]);
    p_accel_values_destination->z = ((raw_values[4]<<8) + raw_values[5]);
